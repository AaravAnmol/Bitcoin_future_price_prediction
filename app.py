...
#load Model 
model = load_model('D:\Project Machine Learning\Bitcoin_Price_prediction_Model.keras')

st.header('Bitcoin Price Prediction Model')
st.subheader('Bitcoin Price Data')
data = pd.DataFrame(yf.download('BTC-INR','2023-01-01','2024-09-03'))
...
for i in range(base_days, len(m)+future_days):
    ...
    m = np.append(m ,pred)
    z = np.append(z, pred)
st.subheader('Predicted Future 30 Days Bitcoin Price')
z = np.array(z)
